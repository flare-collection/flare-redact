...
// map[string]any{"parola": "hunter2"} be redacted in an Italian codebase.

var secretKeywords = dedupeLower([]string{
	// 1. English
...
func dedupeLower(words []string) map[string]struct{} {
...
// SecretKeywords returns the multilingual key-name vocabulary, sorted for
// stable output.
func SecretKeywords() []string {
	out := make([]string, 0, len(secretKeywords))
	for word := range secretKeywords {
		out = append(out, word)
	}
	sort.Strings(out)
	return out
...
// sensitiveKeyPattern covers the English key names that are sensitive by
// convention, including their punctuated spellings.
var sensitiveKeyPattern = regexp.MustCompile(`^(?i:pass(?:word|wd)?|secret|token|api[_-]?key|access[_-]?key|client[_-]?secret|private[_-]?key|auth(?:orization)?|cookie|session[_-]?id|refresh[_-]?token|credit[_-]?card|card[_-]?number|cvv|ssn)$`)

func isSensitiveKeyName(name string) bool {
	if sensitiveKeyPattern.MatchString(name) {
		return true
	}
	_, ok := secretKeywords[strings.ToLower(name)]
	return ok
}
